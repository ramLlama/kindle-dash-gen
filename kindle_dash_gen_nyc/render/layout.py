...
from .toolkit import Fonts, LayoutError

__all__ = ["Layout", "LayoutError", "register_layout", "render"]


class Layout(Protocol):
    """The interface a layout class implements: construct with panel size + fonts, then render."""
...
def render(data: DashboardData, *, units: str, width: int, height: int, layout: str,
    ...
    image = _LAYOUTS[layout](width, height, Fonts(font), units).render(data)
    buffer = BytesIO()
